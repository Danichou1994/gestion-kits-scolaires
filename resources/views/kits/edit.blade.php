    ...
    <form action="{{ route('kits.update', $kit) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="nom_kit" class="w-full border rounded-lg px-3 py-2" value="{{ old('nom_kit', $kit->nom_kit) }}" required>
                @error('nom_kit')
            ...
            <div>
                ...
                <input type="text" name="description" class="w-full border rounded-lg px-3 py-2" value="{{ old('description', $kit->description) }}">
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            <div id="articles-container">
                ...
                <div class="flex gap-4 mb-2 article-row">
                    <select name="articles[{{ $index }}][id]" class="flex-1 border rounded-lg px-3 py-2" required>
                        <option value="">Sélectionner</option>
                        @foreach($articles as $a)
                        <option value="{{ $a->id }}" {{ $a->id == $article->id ? 'selected' : '' }}>
                            {{ $a->nom_article }} ({{ number_format($a->prix_vente) }} F)
                        </option>
                        @endforeach
                    </select>
                    <input type="number" name="articles[{{ $index }}][quantite]" value="{{ $article->pivot->quantite }}" min="1" class="w-24 border rounded-lg px-3 py-2" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
            <div>
                ...
                <input type="number" name="reduction" id="reduction" class="w-full border rounded-lg px-3 py-2" value="{{ old('reduction', $kit->reduction) }}" min="0" step="0.01">
            </div>
            <div>
                <label class="block text-gray-700 mb-2">Frais livraison</label>
                <input type="number" name="frais_livraison" id="frais_livraison" class="w-full border rounded-lg px-3 py-2" value="{{ old('frais_livraison', $kit->frais_livraison) }}" min="0" step="0.01">
            </div>
            <div>
                <label class="block text-gray-700 mb-2">Frais carnet</label>
                <input type="number" name="frais_carnet" id="frais_carnet" class="w-full border rounded-lg px-3 py-2" value="{{ old('frais_carnet', $kit->frais_carnet) }}" min="0" step="0.01">
            </div>
            <div>
                <label class="block text-gray-700 mb-2">Frais emballage</label>
                <input type="number" name="frais_emballage" id="frais_emballage" class="w-full border rounded-lg px-3 py-2" value="{{ old('frais_emballage', $kit->frais_emballage) }}" min="0" step="0.01">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <div>
                ...
                <select name="en_promotion" class="w-full border rounded-lg px-3 py-2">
                    <option value="0" {{ !$kit->en_promotion ? 'selected' : '' }}>Non</option>
                    <option value="1" {{ $kit->en_promotion ? 'selected' : '' }}>Oui</option>
                </select>
            </div>
            <div>
                <label class="block text-gray-700 mb-2">Date début promo</label>
                <input type="date" name="date_debut_promo" class="w-full border rounded-lg px-3 py-2" value="{{ old('date_debut_promo', $kit->date_debut_promo) }}">
            </div>
            <div>
                <label class="block text-gray-700 mb-2">Date fin promo</label>
                <input type="date" name="date_fin_promo" class="w-full border rounded-lg px-3 py-2" value="{{ old('date_fin_promo', $kit->date_fin_promo) }}">
            </div>
        </div>
        ...
        <div class="bg-gray-50 rounded-lg p-4 mt-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <div class="text-xl font-bold" id="prix_total_display">{{ number_format($kit->prix_total ?? 0) }} F</div>
                </div>
                <div>
                    <span class="text-gray-600">Réduction</span>
                    <div class="text-xl font-bold text-green-600" id="reduction_display">{{ number_format($kit->reduction ?? 0) }} F</div>
                </div>
                <div>
                    <span class="text-gray-600">Prix final</span>
                    <div class="text-2xl font-bold text-blue-600" id="prix_final_display">{{ number_format($kit->prix_final ?? 0) }} F</div>
                </div>
            </div>
        </div>
